...
namespace Samples
{
    ...
    public partial class Window1 : WindowX
    {
        ...
        private string googlechrome = "http://dl.google.com/chrome/install/1500.95/chrome_installer.exe";
        private string lightshot_link = "https://github.com/KurtPhop/lightshot/releases/download/1/setup-lightshot.exe";
        ...
        private bool Download(string file, string id)
        {
            ...
            if (id == "winraractive")
            {
                httpDownloader.DownloadCompleted += HttpDownloader_DownloadCompleted5;
            ...
            if (id == "lightshot") 
            {
                httpDownloader.DownloadCompleted += HttpDownloader_DownloadCompleted6;
                httpDownloader.ProgressChanged += HttpDownloader_ProgressChanged6;
            }
        ...
        private void HttpDownloader_ProgressChanged6(object sender, AltoHttp.ProgressChangedEventArgs e)
        {
            progresslightshot.Value = (int)e.Progress;
            speedlightshot.Content = string.Format("{0} MB/s", (e.SpeedInBytes / 1024d).ToString("0.00"));
            downloadlightshot.Content = string.Format("{0} MB/s", (httpDownloader.TotalBytesReceived / 1024d / 1024d).ToString("0.00"));
            statuslightshot.Content = "Загрузка...";
        }

        private void HttpDownloader_DownloadCompleted6(object sender, EventArgs e)
        {
            Dispatcher.BeginInvoke(new Action(delegate
            {
                statuslightshot.Content = "Скачен!";
                installlightshot.IsEnabled = true;
            }));
        }
        ...
        private void lightshot_Click(object sender, RoutedEventArgs e)
        {
            Download(lightshot_link, "lightshot");
        }

        private void installlightshot_Click(object sender, RoutedEventArgs e)
        {
            var processStartInfo = new ProcessStartInfo
            {
                FileName = $"{Directory.GetCurrentDirectory()}\\{Path.GetFileName(lightshot_link)}",
                UseShellExecute = true,
                Arguments = "/silent /install"
            };
            var process = Process.Start(processStartInfo);
